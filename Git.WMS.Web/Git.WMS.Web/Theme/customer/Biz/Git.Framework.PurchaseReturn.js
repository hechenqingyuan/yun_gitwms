...
var PurchaseReturnManager = {
    TabGrid:undefined,
    Server: function () {
        var config = (function () {

            var URL_GetList = "/Biz/PurchaseReturnManagerAjax/GetList";
            var URL_Delete = "/Biz/PurchaseReturnManagerAjax/Delete";
            var URL_Cancel = "/Biz/PurchaseReturnManagerAjax/Cancel";
            var URL_Audite = "/Biz/PurchaseReturnManagerAjax/Audite";
            var URL_GetDetail = "/Biz/PurchaseReturnManagerAjax/GetDetail";
            var URL_ToExcel = "/Biz/PurchaseReturnManagerAjax/ToExcel";
    ...
    PageClick:function(PageIndex,PageSize){
        ...
        var Server=PurchaseReturnManager.Server();
        var search=PurchaseReturnManager.GetSearch();
        search["PageIndex"]=PageIndex;
        search["PageSize"]=PageSize;
        Server.GetList(search,function(result){
            $.jBox.closeTip();
            if(result.Code==1){
                PurchaseReturnManager.SetTable(result);
            }else{
    ...
    SetTable:function(result){
        var cols=[
            ...
            {title:'采购单号', name:'PurchaseOrderNum', width: 80, align: 'center',lockWidth:false,  renderer: function(data,item,rowIndex){
                return data;
            }},
            ...
            {title:'状态', name:'Status', width: 80, align: 'center',lockWidth:false,  renderer: function(data,item,rowIndex){
                ...
                return git.GetEnumDesc(EPurchaseReturnStatus,data);
            }},
            ...
            {title:'操作', name:'ID', width: 150, align: 'left',lockWidth:false,  renderer: function(data,item,rowIndex){
                ...
                if(EPurchaseReturnStatusJson.CreateOrder==item.Status){
        ...
        if(this.TabGrid==undefined){
            ...
            //绑定事件
            PurchaseReturnManager.BindEvent();
        }else{
            this.TabGrid.load(result.Result);
        }
        var pageInfo=result.PageInfo;
        if(pageInfo!=undefined){
            $("#mypager").pager({ pagenumber: pageInfo.PageIndex, recordCount: pageInfo.RowCount, pageSize: pageInfo.PageSize, buttonClickCallback: PurchaseReturnManager.PageClick });
        }
    },
    BindEvent:function(){
        this.TabGrid.on("cellSelected",function(e, item, rowIndex, colIndex){
            if($(e.target).is("a.edit")){
                ...
                var submit=function(v,h,f){
                    if(v=="ok"){
                        ...
                        Server.Delete(param,function(result){
                            if(result.Code==1){
                                var pageSize=$("#mypager").pager("GetPageSize");
                                PurchaseReturnManager.PageClick(1,pageSize);
                            }else{
                                $.jBox.tip(result.Message,"warn");
                            }
                        });
                ...
                //--采购单查看详细
                var SN=item.OrderSnNum;
                PurchaseReturnManager.Detail(SN,"View");
            }
        });
    },
    GetSelect:function(){
    ...
    GetSearch:function(){
        ...
        var PurchaseOrderNum=searchBar.find("input[name='PurchaseOrderNum']").val();
        ...
        search["PurchaseOrderNum"]=PurchaseOrderNum;
    ...
    Detail:function(SnNum,Command){
        ...
        if(Command=="View"){
            $.jBox.open("get:/Biz/PurchaseReturn/Detail?SnNum="+SnNum, "采购退货单详细", 850, 450, { buttons: {"关闭":4 } ,submit:submit,loaded:function(h){
                load(h);
            }}); 
        }else if(Command=="Audite"){
            $.jBox.open("get:/Biz/PurchaseReturn/Detail?SnNum="+SnNum, "采购退货单详细", 850, 450, { buttons: {"审核通过": 1, "审核不通过": 2,"打印":3,"关闭":4 } ,submit:submit,loaded:function(h){
                load(h);
            }}); 
        }
    },
    Init:function(){
        ...
        $("div.toolbar").find("a.btn").click(function(){
            ...
            if(command=="Add"){
                ...
                var list=PurchaseReturnManager.GetRows();
                if(list.length==0){
                ...
                var submit=function(v,h,f){
                    if(v=="ok"){
                        var list=PurchaseReturnManager.GetSelect();
                        if(list.length==0){
        ...
        searchBar.find("a[data-command='search']").click(function(){
            PurchaseReturnManager.PageClick(1,10);
        });
